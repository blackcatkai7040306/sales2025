...
// Third-party Imports
import ReactPlayer from '@/components/libs/ReactPlayer'

// Type Imports
import type { Course } from '@/types/academyTypes'
    ...
    <Card>
      <CardContent className='flex flex-wrap items-center justify-between gap-4'>
        ...
        <div className='flex items-center gap-4'>
          <Chip label ={course?.tags} variant='tonal' size='small' color='error' />
      ...
      <CardContent>
        <div className='border rounded'>
          <div className='mli-2 mbs-2 overflow-hidden rounded'>
           <img src={course?.tutorImg} alt={course?.courseTitle} className='is-full' />
          </div>
          <div className='flex flex-col gap-6 p-6'>
              <div className='flex flex-wrap max-md:flex-col justify-between gap-6'>
               <div className='flex gap-4'> 
                <div className='flex items-center gap-x-2'>
                  <Typography variant='h2'>{course?.rating}</Typography>
                  <Rating value={course?.rating} emptyIcon={<i className='bx-bxs-star' />} />
                </div>
                <div className='flex gap-x-2 mbs-1'>
                  <Chip label={course?.rating} size='small' color='primary' variant='tonal' />
                </div>
                </div>
                {data.map((item, i) => (
            ...
            <div className='flex flex-col gap-4'>
              ...
              {/* {course?.description.map((value, index) => <Typography key={index}>{value}</Typography>)} */}
